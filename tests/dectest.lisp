...
(defun make-line-lexer (string)
  (let* ((length (length string))
    ...
    (named-lambda line-parser ()
      (flet ((get-value ()
               (let ((*package* (find-package :decimal-floats-tests)))
...
(defun get-operation (operation)
  (case operation
    ...
    (comparetotmag #'compare-total-abs)
    (copy #'copy-decimal)
    (copyabs #'copy-abs)
    (copynegate #'copy-minus)
    (copysign #'copy-sign)
...
(defun convert-testcase-file (input &key (eval-p nil)
  ...
  (let* ((input (pathname input))
         ...
         (*testcase-rounding-mode* :half-up)
    ...
    (format *trace-output* ";;; Processing of file ~S has started.~%" input)
...
-*-~%~%;;; This file was automatically generated by CONVERT-TESTCASE-FILE.~%~{~%~S~%~}"
                    parsed-output))))
      (format *trace-output* ";;; Processing of file ~S has finished.~%" input)
      (when eval-p
...
version being used: ~S, ignored version: ~S."
         ...
         (t (error "Unknown directive found during testcase parsing: ~S." directive)))
       nil)))
...
(defun result-to-string (result operation)
  (etypecase result
    (decimal-float (decimal-to-string result
                                      :format (case operation
                                                (tosci :scientific)
                                                (toeng :engineering)
                                                (t :scientific))))
    (string result)
    (boolean (if result "1" "0"))))
  ...
  (multiple-value-bind (new-result new-conditions)
      (with-condition-trap-enablers (nil)
        (with-condition-flags (nil)
          (values (apply (get-operation operation)
                         (mapcar (rcurry #'parse-decimal
                                         :round-p (member operation '(tosci toeng apply)))
                                 operands))
                  (get-condition-flags))))
    (ensure-same
     (result-to-string new-result operation) result
     :test #'equal
